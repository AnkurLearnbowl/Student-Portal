...
import "./liveclassrecordings.css";
import {
...
function LiveClassRecordings() {
  ...
  const regularVideos = JSON.parse(sessionStorage.getItem("regularLectures"));
  const courseOpted = sessionStorage.getItem("courseOpted");
  //console.log(courseOpted);
  const regularFrenchVideos = regularVideos[0]?.French;
  const regularVedicVideos = regularVideos[1]?.Vedic;
  ...
  if (courseOpted === "French") {
    vidoesToShow = regularFrenchVideos;
  } else {
    vidoesToShow = regularVedicVideos;
  }
  console.log(vidoesToShow);
